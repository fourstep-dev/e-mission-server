...
class SmoothBoundary(object):
    ...
    def filter(self, with_speeds_df):
        ...
        for i, pt in enumerate(
            with_speeds_df[["mLatitude", "mLongitude", "mTime", "speed"]].to_dict(
                "records"
            )
        ):
            ...
            if prev_pt is None:
                ...
                if currSpeed > self.maxSpeed:
                    logging.debug(
                        "currSpeed > %s, removing index %s " % (self.maxSpeed, i)
                    )
                    self.inlier_mask_[i] = False
                else:
                    logging.debug(
                        "currSpeed < %s, retaining index %s " % (self.maxSpeed, i)
                    )
                    prev_pt = pt
        logging.info(
            "Filtering complete, removed indices = %s" % np.nonzero(self.inlier_mask_)
        )
...
class Segment(object):
    ...
    def __init__(self, start, end, zigzag_algo):
        ...
        logging.debug(
            "For cluster %s - %s, distance = %s, is_cluster = %s"
...
class SmoothZigzag(object):
    ...
    def shortest_non_cluster_segment(segment_list):
        ...
        segment_distance_list = [
            [segment.distance, segment.is_cluster] for segment in segment_list
        ]
        segment_distance_df = pd.DataFrame(
            segment_distance_list, columns=["distance", "is_cluster"]
        )
        non_cluster_segments = segment_distance_df[
            segment_distance_df.is_cluster == False
        ]
    ...
    def find_segments(self):
        ...
        if last_point not in segmentation_points:
            logging.debug(
                "smoothing: last_point index %s not in found points %s"
    ...
    def split_segment(self, i, curr_seg, direction):
        ...
        if direction == SmoothZigzag.Direction.RIGHT:
            ...
            new_split_point = recomputed_speed_df[
                recomputed_speed_df.distance > Segment.CLUSTER_RADIUS
            ].index[0]
    ...
    def toggle(expected_state):
        assert (
            expected_state == Segment.State.BAD or expected_state == Segment.State.GOOD
        ), "Unable to toggle %s " % expected_state
    ...
    def mark_segment_states(self, start_segment_idx, direction):
        ...
        while check(i):
            ...
            logging.debug(
                "Processing segment %d: %s, expecting state %s"
                % (i, curr_seg, expected_state)
            )
            assert curr_seg.state == Segment.State.UNKNOWN, (
                "Attempting to overwite state for segment %s, curr state is %s"
                % (i, curr_seg.state)
            )

            if (
                expected_state == Segment.State.BAD and not curr_seg.is_cluster
            ):  # mixed cluster case
                ...
                if direction == SmoothZigzag.Direction.LEFT:
                    i = i + 1
                logging.debug(
                    "Finishing process for %s after splitting mixed cluster" % curr_seg
                )
                assert curr_seg.is_cluster, (
                    "after splitting, the segment is not a cluster?!"
                )
            ...
            logging.debug(
                "At the end of the loop for direction %s, i = %s" % (direction, i)
            )
    ...
    def filter(self, with_speeds_df):
        ...
        self.find_segments()
        logging.debug(
            "After splitting, segment list is %s with size %s"
        ...
        assert len(unknown_segments) == 0, (
            "Found %s unknown segments - early termination of loop?"
        ...
        self.inlier_mask_ = self.inlier_mask_.to_numpy()
        logging.debug(
            "after setting values, outlier_mask = %s"
            % np.nonzero(np.logical_not(self.inlier_mask_))
        )
    ...
    def filter(self, with_speeds_df):
        ...
        for curr_segment in quality_segments[1:]:
            ...
            if len(curr_segment) == 0:
                ...
                continue
            get_coords = lambda i: [
                with_speeds_df.iloc[i]["longitude"],
                with_speeds_df.iloc[i]["latitude"],
            ]
            ...
            if get_ts(curr_segment[-1]) - get_ts(curr_segment[0]) <= get_ts(
                last_segment[-1]
            ) - get_ts(last_segment[0]):
                ...
                for curr_idx in curr_segment:
                    print(
                        ...
                        % (
                            math.fabs(
                                ec.calDistance(
                                    get_coords(ref_idx), get_coords(curr_idx)
                                )
                            ),
                            self.maxSpeed / 100,
                    ...
                    if math.fabs(
                        ec.calDistance(get_coords(ref_idx), get_coords(curr_idx))
                    ) > (
        ...
        logging.info(
            "Filtering complete, retained indices = %s, removed indices = %s"
...
class SmoothPiecewiseRansac(object):
    ...
    def filter_area_using_ransac(self, area_df):
        ...
        logging.debug(
            "In area %s - %s, deleted %d points through ransac filtering"
            % (
                area_df.index[0],
                area_df.index[-1],
                np.count_nonzero(np.logical_not(inlier_mask)),
            )
        )
        return inlier_mask
    ...
    def find_areas_of_interest(self, with_speeds_df):
        ...
        logging.debug(
            "Found %d potential outliers, list = %s"
            % (len(candidateIndices), candidateIndices)
        )
        if len(candidateIndices) == 0:
            logging.info(
                "No potential outliers (%s), so no areas to consider", candidateIndices
            )
            return []
        if len(candidateIndices) == 1:
            candidateClusterCenters = [candidateIndices]
            logging.debug(
                "Only one candidate, cluster centers are %s" % candidateClusterCenters
            )
        else:
            ...
            logging.debug(
                "Found %d clusters with centers %s"
                % (len(candidateClusterCenters), candidateClusterCenters)
            )
        dfList = []
        for cc in candidateClusterCenters:
            ...
            logging.debug(
                "lowRange = max(%s, %s) = %s and highRange = max(%s, %s) = %s"
                % (
                    cc[0] - 5,
                    0,
                    lowRange,
                    cc[0] + 5,
                    with_speeds_df.shape[0],
                    highRange,
                )
            )
    ...
    def filter(self, with_speeds_df):
        ...
        for area in areas_of_interest:
            logging.debug(
                "Area size = %s, index = %s with size %s"
                % (area.shape[0], area.index, len(area.index))
            )
        ...
        logging.debug(
            "with speed df shape is %s, ransac_mask size = %s"
            % (with_speeds_df.shape, len(ransac_mask))
        )
        logging.debug(
            "filtering done, ransac deleted points = %s"
            % np.nonzero(ransac_mask == False)
        )
